...
$(document).on("keyup blur focus keydown", "input:text[typeNum1]", function(){
	$(this).val( $(this).val().replace(/[^0-9\-\+]/gi,"") ); //숫자+-만
});
$(document).on("keyup blur focus keydown", "input:text[typeFloat]", function(){
	$(this).val( $(this).val().replace(/[^0-9\.]/gi,"") ); //숫자.만
});
...
function rseom() {
	console.log('%c Made by. Rseom ', 'background-color:green; color:#ffffff; font-size: 30px;');
	console.log('rseom homepage. http://rseom.ml');
}
